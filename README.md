...
> *这是希望的春天，也是失望的冬天；我们将拥有一切，我们将一无所有；*
> *我们直升天堂，我们直下地狱。*
>
> — 查尔斯·狄更斯《双城记》
>
...
- [PAL 架构哲学](#pal-架构哲学)
  - [核心断言](#核心断言)
  - [四条铁律](#四条铁律)
  - [RCI vs RPC](#pal-交互的本质远程代码注入rci)
  - [三组定位对比](#三组定位对比)
- [PAL 的范围：不是什么，是什么](#pal-的范围不是什么是什么)
- [1. 设计原则](#1-设计原则)
  - [开发板 vs 工程项目：两种 Agent 操作模式](#开发板-vs-工程项目两种-agent-操作模式)
- [2. 架构](#2-架构)
  - [2.0 Level 1 — 单核直连](#20-level-1--单核-micropython-直连)
  - [2.1 Level 2 — 双核分离](#21-level-2--双核分离大脑全知)
  - [2.2 总览](#22-总览)
  - [2.3 Core 0 — 大脑：四大组件](#23-core-0--大脑四大组件)
  - [2.4 Core 1 — 手：单任务执行器](#24-core-1--手单任务执行器)
  - [2.5 虚拟串口](#25-虚拟串口level-1--level-2-零改动迁移)
  - [2.6 相位锁定调度](#26-相位锁定调度)
  - [2.7 PAL vs FreeRTOS](#27-pal-调度器-vs-freertos-调度器)
  - [2.8 组件清单](#28-组件清单)
  - [2.9 组件交互图](#29-组件交互图)
- [3. PAL 协议](#3-pal-协议)
  - [3.1 消息格式](#31-消息格式)
...
- 适合开发调试、教育实验、个人 DIY
- **Level 1 代码（`pal_main.py`）可直接迁移到 Level 2 的 Core 1，只需换串口对象**

### 2.1 Level 2 — 双核分离·大脑全知

> **PAL Level 2 的整个架构建立在"大脑全知，手零自主"的核心理念之上。**
> Core 0（大脑，C / FreeRTOS）掌握所有任务信息、资源、时间表，做所有决策。
> Core 1（手，MicroPython）阻塞等待，收到 code → exec → 返回，零自主意识。
...
### 2.5 虚拟串口：Level 1 → Level 2 零改动迁移
